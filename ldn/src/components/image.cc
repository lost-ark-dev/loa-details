...
bool Image::canFocus() { return false; }
...
void Image::decode(RenderContext *ctx) {
  ...
  unsigned error =
      lodepng::decode(decoded_data, native_width, native_height, encoded_data);
  if (error != 0) {
    if (error == 28) {
      uint8_t *decoded =
          WebPDecodeRGBA(&encoded_data[0], encoded_data.size(),
                         (int *)&native_width, (int *)&native_height);
      decoded_data.insert(decoded_data.begin(), decoded,
                          decoded + (native_width * native_height * 4));
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, (GLsizei)native_width,
               (GLsizei)native_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
