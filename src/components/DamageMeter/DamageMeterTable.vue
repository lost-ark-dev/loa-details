      ...
      <thead>
        <q-menu touch-position context-menu>
          <q-list dense style="min-width: 100px">
            ...
            >
              <q-item-section side>
                <q-icon v-if="settingsStore.damageMeter.tabs[tabName]" name="check" />
        ...
        <tr v-if="focusedPlayer === '#'">
          ...
          <template
            v-if="
              ['dmg', 'tank', 'heal', 'shield_given', 'shield_gotten', 'eshield_given', 'eshield_gotten'].includes(
                damageType
              )
            "
          >
            <th v-if="settingsStore.damageMeter.tabs.deathTime" style="width: 48px">Dead for</th>
            <th v-if="settingsStore.damageMeter.tabs.damage" style="width: 72px">
              {{
                damageType === "dmg"
                  ? "Damage"
                  : damageType === "tank"
            ...
            <th v-if="settingsStore.damageMeter.tabs.damagePercent" style="width: 48px">
              {{
                damageType === "dmg"
                  ? "D"
                  : damageType === "tank"
            ...
            <th v-if="settingsStore.damageMeter.tabs.dps" style="width: 52px">
              {{
                damageType === "dmg"
                  ? "DPS"
                  : damageType === "tank"
          ...
          <template
            ...
            "
          >
            <th v-if="settingsStore.damageMeter.tabs.damage" style="width: 72px">Damage</th>
            <template
              v-if="
            ...
            >
              ...
              >
                <div class="header_text">
                  {{ getClassName(columnData.values().next().value.source.skill?.classid) }}
                </div>
            ...
            >
              ...
              >
                <div class="header_text">
                  {{
                    columnKey.includes("_") ? columnKey.split("_")[1] : columnData.values().next().value.buffcategory
                  }}
                </div>
                <div class="header_container">
                  <template v-for="[buffId, buffData] of columnData" :key="buffId">
          ...
          <th v-if="damageType === 'dmg' && settingsStore.damageMeter.tabs.critRate" style="width: 48px">CRIT</th>
          <th v-if="damageType === 'dmg' && settingsStore.damageMeter.tabs.faRate" style="width: 48px">F.A.</th>
          <th v-if="damageType === 'dmg' && settingsStore.damageMeter.tabs.baRate" style="width: 48px">B.A.</th>
          <th v-if="damageType === 'dmg' && settingsStore.damageMeter.tabs.hBuffedBySup" style="width: 52px">HBuf%</th>
          <th v-if="damageType === 'dmg' && settingsStore.damageMeter.tabs.hDebuffedBySup" style="width: 52px">
            HDebuf%
          </th>
          <th v-if="damageType === 'dmg' && settingsStore.damageMeter.tabs.dBuffedBySup" style="width: 52px">DBuf%</th>
          <th v-if="damageType === 'dmg' && settingsStore.damageMeter.tabs.dDebuffedBySup" style="width: 52px">
            DDebuf%
          </th>
          <th v-if="damageType === 'dmg' && settingsStore.damageMeter.tabs.counterCount" style="width: 44px">CNTR</th>
          <template v-if="['shield_given', 'shield_gotten'].includes(damageType)">
            ...
            >
              <div class="header_text">
                {{ getClassName(columnData.source.skill?.classid) }}
              </div>
              <div class="header_container">
                <div>
                  <img class="header_img" :src="getIconPath(columnData.source.icon)" />
        ...
        <tr v-else-if="focusedPlayer !== '#'">
          ...
          <template v-if="['dmg', 'tank', 'heal', 'shield_given'].includes(damageType)">
            <th v-if="settingsStore.damageMeter.tabs.damagePercent" style="width: 48px">D%</th>
            <th v-if="settingsStore.damageMeter.tabs.dps" style="width: 52px">DPS</th>
            <th v-if="settingsStore.damageMeter.tabs.critRate" style="width: 48px">CRIT</th>
            <th v-if="settingsStore.damageMeter.tabs.faRate" style="width: 48px">F.A.</th>
            <th v-if="settingsStore.damageMeter.tabs.baRate" style="width: 48px">B.A.</th>
            <th v-if="settingsStore.damageMeter.tabs.hBuffedBySup" style="width: 52px">HBuf%</th>
            <th v-if="settingsStore.damageMeter.tabs.hDebuffedBySup" style="width: 52px">HDebuf%</th>
            <th v-if="settingsStore.damageMeter.tabs.dBuffedBySup" style="width: 52px">DBuf%</th>
            <th v-if="settingsStore.damageMeter.tabs.dDebuffedBySup" style="width: 52px">DDebuf%</th>
            <th v-if="settingsStore.damageMeter.tabs.maxDmg" style="width: 52px">MaxDmg</th>
            <th v-if="settingsStore.damageMeter.tabs.avgDmg" style="width: 52px">AvgDmg</th>
            <th v-if="settingsStore.damageMeter.tabs.avgCast" style="width: 52px">AvgCast</th>
            <th v-if="settingsStore.damageMeter.tabs.totalHits" style="width: 52px">TotalHits</th>
            <th v-if="settingsStore.damageMeter.tabs.totalCasts" style="width: 52px">TotalCasts</th>
            <th v-if="settingsStore.damageMeter.tabs.hpm" style="width: 52px">Hits/m</th>
            <th v-if="settingsStore.damageMeter.tabs.cpm" style="width: 52px">Casts/m</th>
          </template>
          <template
            v-else-if="
              [
...
function toggleTabDisplay(tabName: tabDisplayKey) {
  settingsStore.damageMeter.tabs[tabName] = !settingsStore.damageMeter.tabs[tabName];
...
function sortEntities() {
  ...
  const res = entitiesCopy
    ...
    .sort((a, b) => {
      if (settingsStore.damageMeter.design.pinUserToTop) {
...
function calculateSkills() {
  ...
  for (const skill of res) {
    skill.damagePercent = ((skill.damageDealt / entity.damageDealt) * 100).toFixed(1);
    skill.relativePercent = ((skill.damageDealt / res[0].damageDealt) * 100).toFixed(1);
  }

  sortedSkills = res;
}

function getPercentage(player: EntityState, dmgType: tabNames, relativeTo: "total" | "top") {
  ...
  let b = 0;
  if (dmgType === "dmg") {
    if (relativeTo === "top") b = props.sessionState.damageStatistics.topDamageDealt;
    else b = props.sessionState.damageStatistics.totalDamageDealt;
  } else if (dmgType === "tank") {
    if (relativeTo === "top") b = props.sessionState.damageStatistics.topDamageTaken;
    else b = props.sessionState.damageStatistics.totalDamageTaken;
  } else if (dmgType === "heal") {
    if (relativeTo === "top") b = props.sessionState.damageStatistics.topHealingDone;
    else b = props.sessionState.damageStatistics.totalHealingDone;
  } else if (dmgType === "shield_given") {
    if (relativeTo === "top") b = props.sessionState.damageStatistics.topShieldDone;
    else b = props.sessionState.damageStatistics.totalShieldDone;
  } else if (dmgType == "shield_gotten") {
    if (relativeTo === "top") b = props.sessionState.damageStatistics.topShieldGotten;
    else b = props.sessionState.damageStatistics.totalShieldDone;
  } else if (dmgType == "eshield_gotten") {
    if (relativeTo === "top") b = props.sessionState.damageStatistics.topEffectiveShieldingUsed;
    else b = props.sessionState.damageStatistics.totalEffectiveShieldingDone;
  } else if (dmgType == "eshield_given") {
    if (relativeTo === "top") b = props.sessionState.damageStatistics.topEffectiveShieldingDone;
    else b = props.sessionState.damageStatistics.totalEffectiveShieldingDone;
  }
...
function filterStatusEffects(
  ...
  // Self synergies
    ...
    if (["self_buff_dmg", "self_buff_hit"].includes(damageType) && focusedPlayer === "#") {
    ...
    if (["self_buff_dmg", "self_buff_hit"].includes(damageType) && focusedPlayer === "#") {
      addStatusEffectIfNeeded(statusEffects, `set_${buff.source.setname}`, id, buff, focusedPlayer, damageType);
    }
...
function isStatusEffectFiltered(damageType: string, statusEffect: StatusEffect) {
  if (!damageType.includes("_")) return false;
  if ((settingsStore.damageMeter.buffFilter[damageType.split("_")[0]] & StatusEffectBuffTypeFlags.ANY) !== 0)
    return true;
  return (settingsStore.damageMeter.buffFilter[damageType.split("_")[0]] & statusEffect.bufftype) !== 0;
}
